...
Asena.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {
    if (match[1] === '') return await message.sendMessage(Lang.NEED_URL + '.install https://gist.github.com/SourceXSL/d717429499f86e7648c9ba9c9496477c')
    try {
    ...
    if (response.statusCode == 200) {
        ...
        try {
            ...
            fs.unlinkSync('/root/SL/plugins/' + plugin_name + '.js')
...
Asena.addCommand({pattern: 'plugin', fromMe: true, desc: Lang.PLUGIN_DESC }, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('SourceXSL') ? msg : inmsg
