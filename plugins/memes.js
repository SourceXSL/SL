...
if (Config.WORKTYPE == 'private') {
    ...
    Asena.addCommand({pattern: 'meme ?(.*)', fromMe: true, desc: Lang.MEMES_DESC}, (async (message, match) => {   
	    ...
	    memeMaker({
            image: location,         
            outfile: 'SL-Android.png',
            topText: topText,
            bottomText: bottomText,
        }, async function(err) {
            if(err) throw new Error(err)
            await message.client.sendMessage(message.jid, fs.readFileSync('SL-Android.png'), MessageType.image, {filename: 'SL-Android.png', mimetype: Mimetype.png, caption: 'Made by SL Android'});
            await info.delete();    
        });
    }));
}
