...
const {Message, StringSession, Image, Video} = require('./SL/');
...
// Sql
const WhatsAsenaDB = config.DATABASE.define('SL', {
    info: {
...
async function whatsAsena () {
    var insult = await axios.get('https://gist.githubusercontent.com/SourceXSL/9d5580d34be1e2869156fdbadb991463/raw/55d05de60cfeec047a8b633c025bdd701134e051/inside.json')
    ...
    conn.on('connecting', async () => {
        console.log(`${chalk.green.bold('SL')}${chalk.blue.bold('Android')}
...
*/  
        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'TR' || config.LANG == 'AZ') { await conn.sendMessage(conn.user.jid, '*SL Android Runs as Public! 🐺*\n\n_Please do not test plugins here. This is your LOG number._\n_You can try commands in any chat :)_\n\n*Your bot works publicly. To change* _.setvar WORK_TYPE:private_ *use command.*\n\n*Thanks for using SL Android Bot 💌*', MessageType.text);
                await git.fetch();
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { await conn.sendMessage(conn.user.jid, '*SL Android Private Olarak Çalışıyor! 🐺*\n\n_Please do not test plugins here. This is your LOG number._\n_You can try commands in any chat :)_\n\n*Botunuz sadece size özel olarak çalışmaktadır. Değiştirmek için* _.setvar WORK_TYPE:public_ *use command.*\n\n*Thanks for using SL Android Bot 💌*', MessageType.text);
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(
            ...
            else { await conn.sendMessage(conn.user.jid, '*SL Android Working as Private! 🐺*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using SL Android 💌*', MessageType.text);
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (error.message.includes('429')) return;
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [SL Android Bot] --*' + 
                                    '\n*SL Android bir hata gerçekleşti!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [SL Android Bot] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [SL Android Bot] --*' + 
                                    '\n*SL Android an error has occurred!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [SL Android Bot] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
